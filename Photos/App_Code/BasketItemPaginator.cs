...
{
	/// <summary>
	/// Responsible for simplifying the process of paginating large amounts of BasketItem's.
	/// </summary>
	public class BasketPaginator
	{
		#region members
		private int _pageSize;
		private List<IBasketItem> _dataSource;
		#endregion
		...
		/// </summary>
		public List<IBasketItem> DataSource
		{
		...
		/// </summary> 
		public BasketPaginator()
		{
			// defaults.
			PageSize = 50;
			CurrentPage = 1;
			UrlFormat = PaginationControlsUrlFormat.QueryString;
		}
		#endregion
		...
		public List<IBasketItem> GetPage(int pageNumber)
		{
			if (pageNumber > TotalPages)
				pageNumber = TotalPages;

			var page = new List<IBasketItem>();
		...
		private string BuildRewrittenUrlPaginatorControls()
		{
			...
			if (CurrentPage > 1)
			{
				if (CurrentPage == 2)
					...
					control.AppendFormat("<b><a href=\"{0}page/{1}\">previous</a></b> | ", url, (CurrentPage - 1).ToString());
			}
			else
			{
			...
			if (CurrentPage < TotalPages)
				control.AppendFormat(" | <b><a href=\"{0}page/{1}\">next</a></b>", url, (CurrentPage + 1).ToString());
			else
